...
class WaveFormView(
  ...
  override fun onDraw(canvas: Canvas) {
    ...
    if (canvasWidth != canvas.width) {
      ...
      barShader = LinearGradient(
          canvasWidth / 2f - 1, 0f, canvasWidth / 2f + 1, 0f,
          blockColorPlayed, blockColor, Shader.TileMode.CLAMP
      )
      blockPaint.shader = barShader
    }
    ...
    if (resampleData.isNotEmpty()) {
      ...
      for (i in Math.max(
          0, (position / 1000f / secPerBlock).toInt() - (position / 1000f / secPerBlock).toInt()
      ) until Math.min(
          (position / 1000f / secPerBlock).toInt()
              + (width / blockWidth).toInt()
              + (position / 1000f / secPerBlock).toInt(),
          resampleData.size
      )) {

        val x = i.toFloat() * blockWidth + offsetX + width / 2

        canvas.drawLine(
            x, height / 2f, x,
            height / 2f - (resampleData[i] / Short.MAX_VALUE) * height / 2 * topBlockScale,
            blockPaint
        )
        canvas.drawLine(
            x, height / 2f + 2, x,
            height / 2f + (resampleData[i] / Short.MAX_VALUE) * height / 2 * bottomBlockScale,
            blockPaint
        )
      }
      ...
      if (showTimeText)
        if (!seeking) {
          canvas.drawRect(
              width / 2f - 100, height / 2f - 50, width / 2f + 100,
              height / 2f + 10, textBgPaint
          )
          canvas.drawText(
              position.toTimeText() + " | " + data?.duration?.toTimeText(), width / 2f,
              height / 2f, textPaint
          )
        } else {
          canvas.drawRect(
          ...
          canvas.drawText(
              seekingPosition.toTimeText() + " | " + data?.duration?.toTimeText(),
